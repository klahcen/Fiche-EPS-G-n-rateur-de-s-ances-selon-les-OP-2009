...
export default function Levels() {
  return (
    <section id="niveaux" className="py-32 px-6 border-t border-white/[0.04]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {LEVELS.map((level, i) => (
            <FadeIn key={level.badge} delay={i * 100}>
              <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-200 h-full flex flex-col">
                <span className="inline-block self-start text-xs px-3 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 mb-4 font-medium">
                  {level.badge}
                </span>
                <p className="text-white/60 text-sm leading-relaxed flex-1 mb-4">
                  {level.oti.length > 120 ? `${level.oti.slice(0, 120)}…` : level.oti}
                </p>
